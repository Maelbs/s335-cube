...
<header>
    <div class="menu-category">
        ...
        <ul class="nav-list">
            ...
            <li class="nav-item">
                <a id="btn-velo" href="{{ route('boutique.index', ['type' => 'Musculaire']) }}"
                    class="link_category">VÉLOS</a>
                            ...
                            @if(isset($menuVelo))
                                @foreach($menuVelo as $root)
                                    <div class="menu-item root-trigger" data-target="subs-velo-{{ $root->id_categorie }}">
                                        <a href="{{ route('boutique.index', ['type' => 'Musculaire', 'cat_id' => $root->id_categorie]) }}"
                                ...
                                @foreach($menuVelo as $root)
                                    <div id="subs-velo-{{ $root->id_categorie }}" class="subs-container d-none">
                                        @foreach($root->enfants as $enfant)
                                            <div class="menu-item sub-trigger"
                                                data-target="models-velo-{{ $enfant->id_categorie }}">
                                                <a href="{{ route('boutique.index', ['type' => 'Musculaire', 'cat_id' => $root->id_categorie, 'sub_id' => $enfant->id_categorie]) }}"
                                                    class="menu-link-reset">
                                            ...
                                            @if($enfant->modeles->isNotEmpty())
                                                <div class="model-grid">
                                                    @foreach($enfant->modeles as $modele)
                                                        <a href="{{ route('boutique.index', ['type' => 'Musculaire', 'cat_id' => $root->id_categorie, 'sub_id' => $enfant->id_categorie, 'model_id' => $modele->id_modele]) }}"
                                                            class="model-link">
                                                            {{ $modele->nom_modele }}
                                                        </a>
                                                    @endforeach
                            ...
                            @if(isset($menuElec))
                                @foreach($menuElec as $root)
                                    <div id="subs-elec-{{ $root->id_categorie }}" class="subs-container d-none">
                                        @foreach($root->enfants as $enfant)
                                            <div class="menu-item sub-trigger"
                                                data-target="models-elec-{{ $enfant->id_categorie }}">
                                                <a href="{{ route('boutique.index', ['type' => 'Electrique', 'cat_id' => $root->id_categorie, 'sub_id' => $enfant->id_categorie]) }}"
                                                    class="menu-link-reset">
                                                    {{ strtoupper($enfant->nom_categorie) }}
                                                </a>
                                                <span>&rsaquo;</span>
            ...
            {{-- 3. MENU ACCESSOIRES --}}
            <li class="nav-item">
                <a id="btn-accessoire" href="{{ route('boutique.index', ['type' => 'Accessoires']) }}"
                    class="link_category">ACCESSOIRES</a>
                <div class="mega-menu-dropdown">
                    <div class="mega-menu-wrapper" id="wrapper-accessoire">
                        ...
                        <div class="col-roots">
                            @if(isset($categorieAccessoires))
                                @foreach($categorieAccessoires as $root)
                                    <div class="menu-item root-trigger"
                                        data-target="sub-acc-{{ $root->id_categorie_accessoire }}">
                                        <a href="{{ route('boutique.index', ['type' => 'Accessoires', 'cat_id' => $root->id_categorie_accessoire]) }}"
                                            class="menu-link-reset">
                                ...
                                @foreach($categorieAccessoires as $root)
                                    <div id="sub-acc-{{ $root->id_categorie_accessoire }}" class="subs-container d-none">
                                        @if($root->enfants->isNotEmpty())
                                            @foreach($root->enfants as $enfant)
                                                <div class="menu-item sub-trigger"
                                                    data-target="grandchild-acc-{{ $enfant->id_categorie_accessoire }}">
                                                    <a href="{{ route('boutique.index', ['type' => 'Accessoires', 'cat_id' => $enfant->cat_id_categorie_accessoire, 'sub_id' => $enfant->id_categorie_accessoire]) }}"
                                                        class="menu-link-reset">
                        ...
                        <div class="col-models-wrapper">
                            @if(isset($categorieAccessoires))
                                @foreach($categorieAccessoires as $root)
                                    @foreach($root->enfants as $enfant)
                                        <div id="grandchild-acc-{{ $enfant->id_categorie_accessoire }}"
                                            class="models-container d-none">
                                            <h3>{{ $enfant->nom_categorie_accessoire }}</h3>
                                            <div class="model-grid">
                                                @if($enfant->enfants && $enfant->enfants->isNotEmpty())
                                                    @foreach($enfant->enfants as $petitEnfant)
                                                        <a href="{{ route('boutique.index', ['type' => 'Accessoires', 'cat_id' => $enfant->cat_id_categorie_accessoire, 'sub_id' => $petitEnfant->cat_id_categorie_accessoire, 'model_id' => $petitEnfant->id_categorie_accessoire]) }}"
                                                            class="model-link">
    ...
    <div class="menu-user">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" onclick="toggleStoreLocator()" width="25" height="25"
            viewBox="0 0 24 24">
            <g>
                <path
                    d="M20.485 3h-3.992l.5 5s1 1 2.5 1a3.23 3.23 0 0 0 2.139-.806a.503.503 0 0 0 .15-.465L21.076 3.5a.6.6 0 0 0-.591-.5Z" />
                <path d="m16.493 3l.5 5s-1 1-2.5 1s-2.5-1-2.5-1V3h4.5Z" />
                <path d="M11.993 3v5s-1 1-2.5 1s-2.5-1-2.5-1l.5-5h4.5Z" />
                <path
                    d="M7.493 3H3.502a.6.6 0 0 0-.592.501L2.205 7.73a.504.504 0 0 0 .15.465c.328.29 1.061.806 2.138.806c1.5 0 2.5-1 2.5-1l.5-5Z" />
        ...
        <div class="icone">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 20 20">
                    <g>
                        <path fill-rule="evenodd"
                            d="M4.828 4.828A5 5 0 1 0 11.9 11.9a5 5 0 0 0-7.07-7.07Zm6.364 6.364a4 4 0 1 1-5.656-5.657a4 4 0 0 1 5.656 5.657Z"
                            clip-rule="evenodd" />
                        <path
                            d="M11.192 12.627a1 1 0 0 1 1.415-1.414l2.828 2.829a1 1 0 1 1-1.414 1.414l-2.829-2.829Z" />
                    </g>
                </svg>
            </a>
            ...
            <div class="cart-dropdown-wrapper">
               <a id="panier" 
                    ...
                    onclick="window.location.href='{{ route('cart.index') }}?t=' + new Date().getTime(); return false;"
                    aria-label="Cart" 
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 48 48">
                        <g>
                            ...
                            <circle cx="13" cy="39" r="3" stroke-linecap="round" stroke-linejoin="round"
                                stroke-width="4" />
                            <circle cx="39" cy="39" r="3" stroke-linecap="round" stroke-linejoin="round"
                                stroke-width="4" />
                        </g>
                    </svg>
                    
                    <span class="cart-count-display"
                            style="position:absolute; top:-5px; right:-5px; background:red; color:white; border-radius:50%; width:16px; height:16px; font-size:10px; justify-content:center; align-items:center; display: {{ (isset($cartItems) && count($cartItems) > 0) ? 'flex' : 'none' }};">
                        {{ isset($cartItems) ? count($cartItems) : 0 }}
                    </span>
                    </a>
                ...
                <div class="cart-preview-box">
                    ...
                    <div class="cart-footer">
                        <div class="cart-total-row">
                            ...
                            <span class="price">{{ number_format($cartTotal ?? 0, 2, ',', ' ') }} € TTC</span>
                        </div>
