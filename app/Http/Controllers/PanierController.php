...
class PanierController extends Controller
{
    public function index()
    {
        ...
        if (Auth::check()) {
            ...
            foreach($cart as $key => &$item) {
                ...
                $stockMax = $this->calculerStockMax($item['reference'], $item['taille'] ?? 'Non renseigné');
                $item['max_stock'] = $stockMax;
            }
            unset($item); 
    ...
    public function add(Request $request, $reference)
    {
        ...
        if (Auth::check()) {
            ...
            if ($ligne) {
                ...
                if ($newQty > $stockMax) $newQty = $stockMax;
    ...
    private function parseKey(string $key): array
    {
        ...
        if ($lastDash === false) return [$key, 'Non renseigné'];
        return [substr($key, 0, $lastDash), substr($key, $lastDash + 1)];
    }

    private function normaliserTaille($taille): string
    {
        if (empty($taille) || $taille === 'Unique' || $taille === 'null') return 'Non renseigné';
        return $taille;
    }
